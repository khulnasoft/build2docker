...
from build2docker.contentproviders import Mercurial
from build2docker.contentproviders.mercurial import args_enabling_topic

SKIP_HG = os.environ.get("BUILD2DOCKER_SKIP_HG_TESTS", "").lower() not in {
    "",
    "0",
    "false",
    "no",
...
skip_if_no_hg_tests = pytest.mark.skipif(
    SKIP_HG,
    reason="BUILD2DOCKER_SKIP_HG_TESTS",
)
...
def test_detect_mercurial(hg_repo_with_content, repo_with_content):
    ...
    assert mercurial.detect("https://github.com/khulnasoft/build2docker") is None
...
def test_ref_topic(hg_repo_with_content):
    ...
    use the environment variable BUILD2DOCKER_SKIP_HG_TESTS.
