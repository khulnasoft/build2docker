...
def test_version(capsys):
    """
    Test passing '--version' to build2docker
    """
...
def test_build_config_priority(tmp_path):
    config_file = str(tmp_path.joinpath("build2docker_config.py"))
    with open(config_file, "w") as f:
        f.write("c.Build2Docker.dry_run = True")
    r2d = make_r2d(["--config", config_file, "."])
    assert r2d.dry_run
    r2d = make_r2d(["--config", config_file, "--build", "."])
    assert not r2d.dry_run
    with open(config_file, "w") as f:
        f.write("c.Build2Docker.dry_run = False")
...
def test_config_priority(tmp_path, trait, arg, default):
    ...
    with open(config_file, "w") as f:
        f.write(f"c.Build2Docker.{trait} = 'config'")
    r2d = make_r2d(["--config", config_file, "."])
