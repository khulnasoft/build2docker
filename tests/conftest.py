...
- If `verify` is found:
    - Run `jupyter-build2docker` on the test directory.
...
- If a `*.repos.yaml` is found:
    - For each entry of the form `{name, url, ref, verify}`
        - Run `jupyter-build2docker` with the `url` and `ref`
...
def make_test_func(args, skip_build=False, extra_run_kwargs=None):
    """Generate a test function that runs build2docker"""
...
class Build2DockerTest(pytest.Function):
    """A pytest.Item for running build2docker"""
    ...
    def repr_failure(self, excinfo):
        ...
        if isinstance(err, SystemExit):
            cmd = f'jupyter-build2docker {" ".join(map(pipes.quote, self.args))}'
...
class LocalRepo(pytest.File):
    def collect(self):
        ...
        # a yaml list with extra arguments to be passed to build2docker
        ...
        yield Build2DockerTest.from_parent(self, name="build", args=args)

        yield Build2DockerTest.from_parent(
            self,
            name=self.path.name,
            args=args + ["./verify"],
            skip_build=True,
...
class RemoteRepoList(pytest.File):
    def collect(self):
        ...
        for repo in repos:
            ...
            yield Build2DockerTest.from_parent(self, name=repo["name"], args=args)
