...
Build2docker officially supports the following versions of Python
...
`base environment <https://github.com/khulnasoft/build2docker/blob/HEAD/build2docker/buildpacks/conda/environment.yml>`_
...
Python itself releases a new version every year now, and build2docker will follow, with the default Python version generally trailing the latest stable version of Python itself by 1-2 versions.
...
docker image used by ``build2docker``. A typical error message will look like
the following::
...
your environment that will work with ``build2docker``.
...
When running build2docker locally you can use the ``-e`` or ``--env`` command-line
flag for each variable that you want to define.

For example ``jupyter-build2docker -e VAR1=val1 -e VAR2=val2 ...``
...
If you pass the ``--debug`` flag to ``build2docker``, it outputs the
...
the ``-e`` flag for environment variables), and run build2docker on a
local repository::

  build2docker -E my-repository/
...
possible (this is why build2docker buildpacks largely build off of patterns that already
...
our own new tool. In the case of build2docker, we spent time integrating with a pre-existing
