...
build2docker scans a repository for particular :ref:`config-files`, such
as ``requirements.txt`` or ``Project.toml``. The collection of files, their contents,
and the resulting actions that build2docker takes is known
...
``apt.txt`` (with ``apt``). While build2docker automates the
