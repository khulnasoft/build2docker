...
and maintaining build2docker. If you'd like more guidance for how
...
We have a lot of tests for various cases supported by build2docker in the `tests/`
...
This section covers the process by which build2docker defines and updates the
...
To update one of the dependencies shared across all `build2docker` builds, you
...
2. Make a pull request with your changes ([link](https://github.com/khulnasoft/build2docker/blob/HEAD/CONTRIBUTING.md#make-a-pull-request))
...
1. There are two files related to conda dependencies. Edit as needed.
   ...
   - `build2docker/buildpacks/conda/environment.yml`
   ...
   - `build2docker/buildpacks/conda/environment.py-2.7.yml`
...
2. Once you edit either of these files to add a new package / bump version on
   ...
   ```bash
   cd ./build2docker/buildpacks/conda/
   python freeze.py
   ```
...
3. After the freeze script finishes, a number of files will have been **modified**.
   ...
   ```
   build2docker/buildpacks/conda/environment*
   ```

   You should **commit all modified files** to git.

4. Make a Pull Request to the `khulnasoft/build2docker` repository, with a description
...
new release on the [build2docker PyPI](https://pypi.org/project/jupyter-build2docker/).
...
release on the [GitHub repository releases page](https://github.com/khulnasoft/build2docker/releases). Once on that page, follow these steps:
...
## Compare generated Dockerfiles between build2docker versions
...
current_version=$(jupyter-build2docker --version | sed s@+@-@)
echo "Comparing $(pwd) (local $current_version vs. $R2D_COMPARE_TO)"
basename="dockerfilediff"

diff_r2d_dockerfiles_with_version () {
    docker run --rm -t -v "$(pwd)":"$(pwd)" --user 1000 khulnasoft/build2docker:"$1" jupyter-build2docker --no-build --debug "$(pwd)" &> "$basename"."$1"
    jupyter-build2docker --no-build --debug "$(pwd)" &> "$basename"."$current_version"

    # remove first line logging the path
    sed -i '/^\[Build2Docker\]/d' "$basename"."$1"
    sed -i '/^\[Build2Docker\]/d' "$basename"."$current_version"
...
Configure the build2docker version you want to compare with your local version in the environment variable `R2D_COMPARE_TO`.
The scripts takes one input: the directory where build2docker should be executed.
