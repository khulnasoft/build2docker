# Design of build2docker

The build2docker buildpacks are inspired by
[Heroku's Build Packs](https://devcenter.heroku.com/articles/buildpacks).
The philosophy for the build2docker buildpacks includes:
...
When designing `build2docker` and adding to it in the future, the
developers are influenced by two primary use cases.
The use cases for `build2docker` which drive most design decisions are:
...
2. Manual image building and running the image from the command line client,
   `jupyter-build2docker`, by users interactively on their workstations

## Deterministic output

The core of `build2docker` can be considered a
...
So if we run `build2docker` on the same directory multiple times, we get the
exact same Dockerfile output.

This provides a few advantages:

1. Reuse of cached built artifacts based on a repository's identity increases
   efficiency and reliability. For example, if we had already run `build2docker`
   ...
   tools (like BinderHub) on top of `build2docker`.
...
1. version of `build2docker` itself
1. versions of the libraries installed by the repository

`build2docker` controls the first two, the user controls the third one. The current
...
The version of `build2docker` used to build an image can influence which packages
...
`build2docker` should do one thing, and do it well. This one thing is:
...
Docker images, `build2docker` provides the additional functionality to support
...
pattern is popular enough to be included inside `build2docker`. Remember that 'yes'
