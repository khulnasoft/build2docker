...
.. note::
   ...
   order to run ``build2docker``. For more information on installing
   ``build2docker``, see :ref:`install`.

``build2docker`` can build a reproducible computational environment for any repository that
follows :ref:`specification`. build2docker is called with the URL of a Git repository,
...
Calling build2docker
===================

build2docker is called with this command::

  jupyter-build2docker <source-repository>
...
to specify its Python environment. Because of this, ``build2docker`` will use
...
configuration files in ``build2docker`` visit :ref:`config-files`.
...
specify the ``branch-name`` or ``commit-hash``. For example::
  ...
  jupyter-build2docker --ref 9ced85dd9a84859d0767369e58f33912a214a3cf https://github.com/norvig/pytudes
...
``build2docker`` will look for configuration files in:
...
by ``build2docker`` to see how to configure the build process.

.. note::

   ``build2docker`` builds an environment with Python 3.7 by default. If you'd
   like a different version, you can specify this in your
   :ref:`configuration files <config-files>`.


Debugging build2docker with ``--debug`` and ``--no-build``
=========================================================
...
To debug the docker image being built, pass the ``--debug`` parameter:
  ...
  .. code-block:: bash
     ...
     jupyter-build2docker --debug https://github.com/norvig/pytudes
...
is for **debugging purposes** of ``build2docker`` only - it can not
be used by docker directly.

  .. code-block:: bash

     jupyter-build2docker --no-build --debug https://github.com/norvig/pytudes


Command line API
================

.. autoprogram:: build2docker.__main__:argparser
  :prog: jupyter-build2docker
